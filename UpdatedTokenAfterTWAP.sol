...
contract LiquidityGeneratorToken is IERC20, Ownable, BaseToken {
    ...
    uint256 public constant BURN_COOLDOWN = 72 hours; // NEW ADDITION
    ...
    // --- TWAP ORACLE VARIABLES ---
    uint32 private _blockTimestampLast;
    uint256 private _priceCumulativeLast;
    uint256 private _twapPrice112x112; 
    uint32 public constant TWAP_WINDOW = 5 minutes;
    bool public twapInitialized;
    bool private _isToken0;
    uint32 public maxTwapAge = 2 hours;
    uint256 public slippageToleranceBps = 500; // 500 basis points = 5%
    // -----------------------------
    ...
    constructor(
        ...
        _isToken0 = address(this) == IUniswapV2Pair(uniswapV2Pair).token0();
    ...
    /**
     * @dev Manually burns a specified amount of tokens from the owner's balance.
     * This action reduces the total supply.
     * Can only be called by the owner once every BURN_COOLDOWN period (72 hours).
     * @param tAmount The amount of tokens (tAmount) to burn.
     */
    function manualBurn(uint256 tAmount) public virtual onlyOwner {
        require(tAmount > 0, "LiquidityGenerator: Burn amount must be greater than zero");
        require(
            block.timestamp >= _lastManualBurnTimestamp + BURN_COOLDOWN,
            "LiquidityGenerator: Manual burn cooldown not met"
        );

        uint256 ownerBalance = balanceOf(owner());
        require(ownerBalance >= tAmount, "LiquidityGenerator: Insufficient balance to burn");

        uint256 currentRate = _getRate();
        uint256 rAmount = tAmount.mul(currentRate);

        _lastManualBurnTimestamp = block.timestamp;

        // Perform the burn by reducing owner's balance and total supply
        _rOwned[owner()] = _rOwned[owner()].sub(rAmount);
        
        // Ensure tOwned is updated if the owner is excluded from rewards
        ...
        emit Transfer(owner(), address(0), tAmount);
        emit ManualBurn(owner(), tAmount);
    }
    ...
    function blacklistAddress(address account) public onlyOwner {
        require(account != address(0), "LiquidityGenerator: Blacklist zero address");
        require(!_isBlacklisted[account], "LiquidityGenerator: Account already blacklisted");
    ...
    function unBlacklistAddress(address account) public onlyOwner {
        require(_isBlacklisted[account], "LiquidityGenerator: Account not blacklisted");
    ...
    /**
     * @dev Updates the internal TWAP oracle. 
     * Only processes the heavy math if TWAP_WINDOW (5 mins) has passed.
     */
    function _updateTWAP() private {
        // Quick exit if pair isn't set yet, saving gas
        if (uniswapV2Pair == address(0)) return; 

        uint32 blockTimestamp = uint32(block.timestamp % 2 ** 32);
        uint32 timeElapsed;
        
        // CRITICAL FIX: Must be unchecked in Solidity 0.8+ to allow safe overflow wrapping
        unchecked {
            timeElapsed = blockTimestamp - _blockTimestampLast; 
        }

        // Only update the snapshot if the 5-minute window has passed
        if (timeElapsed >= TWAP_WINDOW) {
            IUniswapV2Pair pair = IUniswapV2Pair(uniswapV2Pair);
            uint256 priceCumulative;

            // 1. Get current cumulative from the pair
            ...
            if (blockTimestampLastPair != blockTimestamp) {
                ...
                unchecked {
                    timeElapsedPair = blockTimestamp - blockTimestampLastPair;
                }
                
                // Using our gas-optimized cached variable
                if (_isToken0) {
                    price0Cumulative += uint256((uint256(reserve1) << 112) / reserve0) * timeElapsedPair;
                } else {
                    price1Cumulative += uint256((uint256(reserve0) << 112) / reserve1) * timeElapsedPair;
                }
            }

            priceCumulative = _isToken0 ? price0Cumulative : price1Cumulative;

            // 3. Calculate the actual TWAP if we have a historical snapshot
            if (twapInitialized) {
                unchecked {
                    // Safe to be unchecked as timeElapsed is guaranteed to be >= 300 here
                    _twapPrice112x112 = (priceCumulative - _priceCumulativeLast) / timeElapsed;
                }
            } else {
                twapInitialized = true; // First snapshot taken
            }

            // 4. Save current state for the next window
