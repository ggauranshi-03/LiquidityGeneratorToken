...
contract LiquidityGeneratorToken is IERC20, Ownable, BaseToken {
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        // Note: Check if tokenAmount > 0 to avoid errors
        if(tokenAmount == 0) return;
        
        uint256[] memory amountsOut = uniswapV2Router.getAmountsOut(tokenAmount, path);
        // Allow for 20% slippage (multiply by 80, divide by 100)
        // This prevents sandwich attacks while keeping transfers working
        uint256 amountOutMin = amountsOut[1].mul(80).div(100); 
